...
setup_data_directory() {
    ...
    if [ -f .env ]; then
        ...
        if [ -z "$db_url" ]; then
            db_url="$(grep -E '^[^#]*DATABASE_URL=' .env 2>/dev/null | tail -n1 | sed -E 's/^[^#]*DATABASE_URL=//' | tr -d '"' | xargs)"
        fi
    fi
    
    if [[ "$db_url" =~ ^postgresql:// ]]; then
        # Using PostgreSQL
        print_success "Call history: PostgreSQL configured (DATABASE_URL set)"
        print_info "  Data directory ($DATA_DIR) will be used for SQLite fallback/migration only"
    else
        # Using SQLite (default)
        print_info "Call history: SQLite mode (data/call_history.db)"
        print_info "  For PostgreSQL, set DATABASE_URL in .env or run: ./scripts/setup_postgres.sh"
        
        # Verify SQLite can work in data directory
        if [ -d "$DATA_DIR" ] && [ -w "$DATA_DIR" ]; then
            print_success "Data directory ready: $DATA_DIR"
            
            # Best-effort: validate we can create an SQLite file
...
os.remove(path)
PY
                    print_success "SQLite test passed - directory is writable"
                else
                    print_warning "SQLite test failed - call history may not work"
...
configure_env() {
    ...
    print_info "  AVA supports PostgreSQL (recommended for production) or SQLite (default)"
    
    local CURRENT_DB_URL
    CURRENT_DB_URL=$(grep -E '^[^#]*DATABASE_URL=' .env 2>/dev/null | tail -n1 | sed -E 's/^[^#]*DATABASE_URL=//' | tr -d '"' | xargs)
    
    if [ -n "$CURRENT_DB_URL" ]; then
        if [[ "$CURRENT_DB_URL" =~ ^postgresql:// ]]; then
            print_success "PostgreSQL already configured: $(echo "$CURRENT_DB_URL" | sed -E 's|://[^@]+@|://***@|')"
            read -p "Keep this configuration? [Y/n]: " keep_db
            keep_db="${keep_db:-Y}"
            if [[ ! "$keep_db" =~ ^[Yy]$ ]]; then
                CURRENT_DB_URL=""
            fi
        else
            print_info "Currently using SQLite (data/call_history.db)"
        fi
    fi
    ...
    if [ -z "$CURRENT_DB_URL" ]; then
        ...
        echo "  1) SQLite (default) - file-based, zero configuration"
        echo "  2) PostgreSQL - recommended for production, better concurrency"
        echo ""
        ...
        if [ "$db_choice" = "2" ]; then
            ...
            print_info "  You can run the interactive setup wizard or enter credentials manually"
            echo ""
            read -p "Run PostgreSQL setup wizard? [Y/n]: " run_pg_setup
            run_pg_setup="${run_pg_setup:-Y}"
            
            if [[ "$run_pg_setup" =~ ^[Yy]$ ]]; then
                if [ -f "./scripts/setup_postgres.sh" ]; then
                    print_info "Launching PostgreSQL setup wizard..."
                    ./scripts/setup_postgres.sh
                else
                    print_warning "setup_postgres.sh not found"
                    print_info "  Manual setup: export DATABASE_URL='postgresql://user:pass@host:port/dbname'"
                    print_info "               Then run: python scripts/init_postgres_schema.py"
                fi
            else
                print_info "Skipping PostgreSQL setup - you can configure it later with:"
                print_info "  ./scripts/setup_postgres.sh"
                print_info "Or set DATABASE_URL manually in .env"
            fi
        else
            print_info "Using SQLite (default) - no additional configuration needed"
            print_info "  Database location: data/call_history.db"
            # Ensure DATABASE_URL is not set or commented out
            if grep -qE '^DATABASE_URL=' .env 2>/dev/null; then
                sed -i.bak -E 's/^DATABASE_URL=/#DATABASE_URL=/' .env
                print_info "  Commented out DATABASE_URL in .env for SQLite mode"
            fi
        fi
    fi
...
exten => s,1,NoOp(Asterisk AI Voice Agent)
 same => n,Answer()
 same => n,Stasis(genie-ai-voice-agent)
 same => n,Hangup()
EOF
