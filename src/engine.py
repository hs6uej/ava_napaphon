...
class Engine:
    ...
    def __init__(self, config: AppConfig):
        ...
        # Set callback for when playback completes (buffer empty)
        # This allows playback manager to trigger audio gating open
        self.streaming_playback_manager.on_playback_complete_callback = self._on_playback_complete
        ...
        # Initialize Audio Gating Manager (for echo prevention in Google Live, OpenAI Realtime)
        self.audio_gating_manager = None
        try:
            logger.info("🔍 Initializing AudioGatingManager...")
            from src.core.audio_gating_manager import AudioGatingManager
            
            # AudioGatingManager can work with or without VAD
            # - With VAD: Supports interrupt detection (user can interrupt agent)
            # - Without VAD: Simple gating (block audio when agent speaks)
            self.audio_gating_manager = AudioGatingManager(vad_manager=self.vad_manager)
            
            if self.vad_manager:
                logger.info("🎛️ Audio gating manager initialized with VAD interrupt detection")
            else:
                logger.info("🎛️ Audio gating manager initialized (simple mode, no interrupt detection)")
        except Exception as e:
            logger.error(f"❌ Failed to initialize audio gating manager: {type(e).__name__}: {str(e)}", exc_info=True)
            self.audio_gating_manager = None
    ...
    async def start(self):
        ...
        logger.info("✅ Providers loaded", provider_count=len(self.providers), providers_list=list(self.providers.keys()))
        ...
        for name, provider_config_data in self.config.providers.items():
            ...
            try:
                ...
                if name == "local":
                    ...
                    logger.info("Loading 'google_live' provider", audio_gating_enabled=self.audio_gating_manager is not None)
                    try:
                        #logger.info("audio_gating_manager:{e}", audio_gating_manager = self.audio_gating_manager)
    ...
    async def _handle_caller_stasis_start_hybrid(self, caller_channel_id: str, channel: dict):
        ...
        logger.info("🎯 HYBRID ARI - Caller channel entered Stasis", 
                    ...
                    #session_id=channel.get('session_id',"N/A"),
        ...
        try:
            resp = await self.ari_client.send_command(
                "GET",
                f"channels/{caller_channel_id}/variable",
                params={"variable": "SESSION_ID"},
                tolerate_statuses=[404],
            )
            session_id = resp.get("value") if isinstance(resp, dict) else "N/A"
            logger.info("🎯 HYBRID ARI - SESSION_ID variable", session_id=session_id)
        except Exception:
            session_id = "N/A"   
        ...
        try:
            resp = await self.ari_client.send_command(
                "GET",
                f"channels/{caller_channel_id}/variable",
                params={"variable": "call_metadata_json"},
                tolerate_statuses=[404],
            )
            call_metadata_json = resp.get("value") if isinstance(resp, dict) else "N/A"
            logger.info("🎯 HYBRID ARI - call_metadata_json variable", call_metadata_json=call_metadata_json)
        except Exception:
            call_metadata_json = "N/A"     
        
        globals_asterisk_instance = ""
        try:
            resp = await self.ari_client.send_command(
                "GET",
                f"channels/{caller_channel_id}/variable",
                params={"variable": "GLOBALS_ASTERISK_INSTANCE"},
                tolerate_statuses=[404],
            )
            globals_asterisk_instance = resp.get("value") if isinstance(resp, dict) else "N/A"
            logger.info("🎯 HYBRID ARI - GLOBALS_ASTERISK_INSTANCE variable", globals_asterisk_instance=globals_asterisk_instance)
        except Exception:
            globals_asterisk_instance = "N/A"      
            
        # --- MULTI-TENANT RESOLUTION ---
        tenant_id = None
        called_number = "N/A"
        try:
            # Try to get the dialed number (DID) to identify the tenant
            # Common variables: __FROM_DID (FreePBX), DIALED_NUMBER (Generic)
            for var in ["__FROM_DID", "DIALED_NUMBER", "DNID"]:
                resp = await self.ari_client.send_command(
                    "GET",
                    f"channels/{caller_channel_id}/variable",
                    params={"variable": var},
            ...
            logger.info("🎯 MULTI-TENANT - Resolving tenant for DID", called_number=called_number)
            tenant_info = await tenant_manager.resolve_tenant_by_did(called_number)
            if tenant_info:
                tenant_id = tenant_info["tenant_id"]
                logger.info("🎯 MULTI-TENANT - Tenant resolved", tenant_id=tenant_id, context=tenant_info["context_name"])
            else:
                logger.warning("🎯 MULTI-TENANT - No tenant found for DID", called_number=called_number)
        except Exception:
            logger.error("🎯 MULTI-TENANT - Failed to resolve tenant", exc_info=True)
        # -------------------------------
        ...
        try:
            ...
            logger.info("🎯 HYBRID ARI - Step 3: ✅ Caller added to bridge", 
                       channel_id=caller_channel_id, 
                       bridge_id=bridge_id,
                       call_metadata = call_metadata_json,
                       )
            ...
            session = CallSession(
                ...
                caller_name=caller_info.get('name'),
                caller_number=caller_info.get('number'),
                called_number=called_number, # Resolved during multi-tenant lookup
                tenant_id=tenant_id,        # Set if DID matched a tenant
                
                ## BEGIN: additional variables, requested by Sombatt 
                session_id = session_id,
                globals_asterisk_instance = globals_asterisk_instance,
                call_metadata = call_metadata_json or "{}",
                ## END:
            ...
            # --- MULTI-TENANT SETTINGS INJECTION ---
            if tenant_id:
                try:
                    settings = await tenant_manager.get_tenant_settings(tenant_id)
                    if settings:
                        logger.info("🎯 MULTI-TENANT - Applying tenant configuration", tenant_id=tenant_id)
                        # Merge tenant settings into provider_overrides.
                        # These will be applied by _apply_provider_overrides when the provider starts.
                        if settings.get("greeting"):
                            session.provider_overrides["greeting"] = settings["greeting"]
                        if settings.get("prompt"):
                            session.provider_overrides["prompt"] = settings["prompt"]
                        if settings.get("transfer_number"):
                            session.provider_overrides["transfer_number"] = settings["transfer_number"]
                except Exception:
                    logger.error("🎯 MULTI-TENANT - Failed to apply tenant settings", exc_info=True)
            # --------------------------------------
                    ...
                    if email_tool:
                        ...
                        if not check_session:
                            ...
                            context = ToolExecutionContext(
                                ...
                                globals_asterisk_instance=getattr(session, 'globals_asterisk_instance', None),
        ...
        try:
            ...
            if continuous_input:
                ...
                try:
                    ...
                    # 🎛️ Audio Gating: Check if audio should be forwarded to provider
                    # This prevents echo by blocking/buffering user audio when agent is speaking
                    if frame_num <= 3 or frame_num % 50 == 0:
                        logger.info(
                            "🔍 Continuous input path - checking gating",
                            call_id=session.call_id,
                            provider_name=provider_name,
                            has_gating_manager=self.audio_gating_manager is not None,
                            frame_num=frame_num,
                        )
                    
                    if self.audio_gating_manager:
                        try:
                            # Determine audio format for gating manager
                            audio_format = "pcm16" if prov_enc in ("linear16", "pcm16", "slin16", "slin") else "ulaw"
                            
                            if frame_num <= 3 or frame_num % 25 == 0:
                                logger.info(
                                    "🎛️ GATING CHECK (continuous input)",
                                    call_id=session.call_id,
                                    provider_name=provider_name,
                            ...
                            should_forward, buffered_chunks = await self.audio_gating_manager.should_forward_audio(
                                call_id=session.call_id,
                                provider_name=provider_name,
                                audio_chunk=prov_payload,
                                audio_format=audio_format
                            )
                            ...
                            if buffered_chunks:
                                logger.info(
                                    "📤 Flushing buffered audio (continuous input)",
                                    call_id=session.call_id,
                                    provider_name=provider_name,
                                    buffer_size=len(buffered_chunks),
                                )
                                for buffered_chunk in buffered_chunks:
                                    try:
                                        await provider.send_audio(buffered_chunk, prov_rate, prov_enc)
                                    except TypeError:
                                        await provider.send_audio(buffered_chunk)
                            
                            # Send current chunk only if gate is open
                            if should_forward:
                                ...
                                if frame_num <= 3 or frame_num % 25 == 0:
                                    logger.info(
                                        "🚪 Audio blocked by gating (agent speaking)",
                                        call_id=session.call_id,
                                        provider_name=provider_name,
                                        frame_num=frame_num,
                                    )
                        except Exception as e:
                            # If gating fails, fall back to sending audio (fail-open for reliability)
                            logger.warning(
                                "⚠️ Audio gating check failed, sending audio anyway",
                                call_id=session.call_id,
                                provider_name=provider_name,
                                error=str(e),
                            )
                        ...
                        # No gating manager, send audio directly
                        if frame_num <= 3:
                            logger.info(
                                "⚠️ No audio gating manager available, sending audio directly",
                                call_id=session.call_id,
                                provider_name=provider_name,
                                frame_num=frame_num,
                            )
            ...
            # 🎛️ Audio Gating: Check if audio should be forwarded to provider
            # This prevents echo by blocking user audio when agent is speaking
            logger.info(
                "🎛️ Audio routing check",
                call_id=session.call_id,
                provider_name=provider_name,
                has_gating_manager=self.audio_gating_manager is not None,
                payload_size=len(provider_payload) if provider_payload else 0,
            )
            
            if self.audio_gating_manager:
                try:
                    # Determine audio format for gating manager
                    # Convert provider encoding to format expected by gating manager
                    audio_format = "pcm16" if provider_encoding in ("linear16", "pcm16", "slin16", "slin") else "ulaw"
                    
                    logger.info(
                        "🎛️ Calling should_forward_audio",
                        call_id=session.call_id,
                        provider_name=provider_name,
                        audio_format=audio_format,
                    )
                    ...
                    logger.info(
                        ...
                        has_buffered=buffered_chunks is not None,
                        buffered_count=len(buffered_chunks) if buffered_chunks else 0,
                    )
                    
                    # Flush any buffered audio first (accumulated while gate was closed)
                    if buffered_chunks:
                        logger.info(
                            "📤 Flushing buffered audio to provider",
                            call_id=session.call_id,
                            provider_name=provider_name,
                            buffer_size=len(buffered_chunks),
                        )
                    ...
                    if should_forward:
                        ...
                        logger.debug(
                            "🚪 Audio blocked by gating manager (agent speaking)",
                            call_id=session.call_id,
                            provider_name=provider_name,
                        )
                except Exception as e:
        ...
        try:
            ...
            if etype in ("ProviderBargeIn", "interruption"):
                try:
                    ...
                    # LAM : when burge-in is detected, we can either flush local output (allowing provider to continue) or fully barge-in (cancel provider).
                    barge_in_enabled = bool(getattr(cfg, "enabled", False))
                    if not barge_in_enabled: 
                        logger.warning("### Engine: barge-in disabled" ,reason=reason)
                        return 
                    # LAM : for now, we always fully barge-in on provider signal since it's more reliable than local VAD and avoids edge cases around partial flushing. Future knob could allow choosing between flush vs full barge-in on provider signal.    
                    await self._apply_barge_in_action(
                        call_id,
            ...
            if etype == "AgentAudio":
                ...
                q = self._provider_stream_queues.get(call_id)
                logger.info("🎯 AgentAudioDone EVENT RECEIVED", call_id=call_id, q="exists" if q else "none", continuous=continuous, provider=getattr(session, "provider_name", None))
                logger.info("1# AgentAudioDone - signaling end of stream", call_id=call_id, q = "exists" if q else "none", continuous=continuous)
                if continuous:
                    ...
                    try:
                        # NOTE: end_segment_gating only clears TTS gating token
                        # Actual unmute + set_agent_speaking(False) happens in playback manager
                        # when buffer is actually empty (_update_idle_tracking -> _on_playback_complete)
                    ...
                    if q is not None:
                        ...
                        logger.info("AgentAudioDone with no active stream queue", call_id=call_id)
                    self._provider_stream_formats.pop(call_id, None)
    ...
    async def _on_playback_complete(self, call_id: str) -> None:
        """Called by StreamingPlaybackManager when playback buffer is empty (agent finished speaking).
        
        This is the correct timing to open the audio gate - when audio has actually
        finished playing, not when the provider finished sending audio.
        ...
        try:
            if self.audio_gating_manager:
                session = await self.session_store.get_by_call_id(call_id)
                if session:
                    provider_name = getattr(session, 'provider_name', 'unknown')
                    self.audio_gating_manager.set_agent_speaking(call_id, False, provider_name)
                    logger.info(
                        "🔓 Audio gate OPENED - playback buffer empty (agent finished speaking)",
                        call_id=call_id,
                        provider=provider_name,
                    )
        except Exception as e:
            logger.error(
                "Error in playback complete callback",
                call_id=call_id,
                error=str(e),
                exc_info=True
            )
    ...
    async def _execute_pre_call_tools(
        ...
        try:
            ...
            pre_call_ctx = PreCallContext(
                ...
                config=self.config.dict() if hasattr(self.config, 'dict') else {},
                ari_client=self.ari_client,
                ## BEGIN: EXPERIMENTAL FIELDS FOR POST-CALL TOOLS (subject to change, not yet documented)
                session_id=getattr(session, 'session_id', 'N/A'),
                globals_asterisk_instance=getattr(session, 'globals_asterisk_instance', 'N/A'),
                ## END:
            )
    ...
    async def _execute_post_call_tools(
        ...
        *,
        ...
        try:
            ...
            call_metadata = None
            if session.call_metadata:
                try:
                    parsed = json.loads(session.call_metadata)
                    call_metadata = parsed if isinstance(parsed, dict) else None
            ...
            #     try:
            #         parsed = json.loads(session.transcription)
            #         transcription = parsed if isinstance(parsed, dict) else None
            #     except json.JSONDecodeError:
            #         transcription = None        
            ...
            post_call_ctx = PostCallContext(
                ...
                ## BEGIN: EXPERIMENTAL FIELDS FOR POST-CALL TOOLS (subject to change, not yet documented)
                session_id=session.session_id if getattr(session, 'session_id', None) else "N/A", #session.session_id,
                globals_asterisk_instance=session.globals_asterisk_instance if getattr(session, 'globals_asterisk_instance', None) else "N/A", #session.globals_asterisk_instance,
                call_metadata=dict(call_metadata) if call_metadata else None,
                ## END:
            )
            ...
            if post_call_ctx.call_metadata:
                logger.info(
                    "Post-call call metadata",
                    call_id=call_id,
                    call_metadata=post_call_ctx.call_metadata)
                
            # if post_call_ctx.transcription:
            #     logger.info(
            #         "Post-call transcription",
            #         call_id=call_id,
            #         transcription=post_call_ctx.transcription)  
