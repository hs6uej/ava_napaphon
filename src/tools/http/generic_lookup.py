...
class GenericHTTPLookupTool(PreCallTool):
        ...
        try:
            ...
            if debug_enabled(logger):
                ...
                ctx_values = {
                    ...
                    "globals_asterisk_instance": getattr(context, "globals_asterisk_instance", None),
                    "call_metadata": dict(getattr(context, "call_metadata", None)),
            ...
            logger.info(f"Executing HTTP lookup: {self.config.name} {self.config.method} {self._redact_url(url)} with headers={{{', '.join(headers.keys())}}} and params={{{', '.join(params.keys())}}}")
    ...
    def _substitute_variables(self, template: str, context: PreCallContext) -> str:
        ...
        replacements = {
            ...
            "{globals_asterisk_instance}": context.globals_asterisk_instance or "N/A",
            "{call_metadata_json}": json.dumps(context.call_metadata) if context.call_metadata else "{}",
            "{call_metadata}": json.dumps(context.call_metadata) if context.call_metadata else "{}",
        }
