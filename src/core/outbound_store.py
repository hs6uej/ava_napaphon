"""
Outbound campaign dialer persistence (SQLite or PostgreSQL).
...
- Atomic lead leasing (transaction-based)
...
def _execute(conn, sql: str, params=None):
    """
    Database-agnostic execute helper.
    SQLite: _execute(conn, ) works directly
    PostgreSQL: Need to create cursor first
    """
    if params is None:
        params = ()
    
    # Adapt placeholders for PostgreSQL
    sql, params = adapt_placeholders(sql, params if isinstance(params, tuple) else tuple(params))
    
    # PostgreSQL requires cursor
    cursor = conn.cursor()
        ...
        def _sync():
            ...
            with self._lock:
                ...
                try:
                    ...
                    sql1, params1 = adapt_placeholders("DELETE FROM outbound_attempts WHERE campaign_id = ?", (campaign_id,))
                    cur.execute(sql1, params1)
                    
                    sql2, params2 = adapt_placeholders("DELETE FROM outbound_leads WHERE campaign_id = ?", (campaign_id,))
                    cur.execute(sql2, params2)
                    
                    sql3, params3 = adapt_placeholders("DELETE FROM outbound_campaigns WHERE id = ?", (campaign_id,))
            ...
            with self._lock:
                ...
                try:
                    ...
                    sql, params = adapt_placeholders(
                        ...
                        """,
                        tuple([lease_until, now, *lead_ids]),
                    )
                    cur.execute(sql, params)
                    conn.commit()

                    data_rows = _execute(conn, 
            ...
            with self._lock:
                ...
                try:
                    ...
                    if m == "reset":
                        ...
                        sql1, params1 = adapt_placeholders("DELETE FROM outbound_attempts WHERE lead_id = ?", (lead_id,))
                        cursor.execute(sql1, params1)
                        
                        sql2, params2 = adapt_placeholders(
                            """
                            UPDATE outbound_leads
                            SET state='pending',
                                attempt_count=0,
        ...
        def _sync():
            with self._lock:
                ...
                try:
                    ...
                    cursor = conn.cursor()
                    sql1, params1 = adapt_placeholders(
                    ...
                    sql2, params2 = adapt_placeholders("DELETE FROM outbound_attempts WHERE lead_id=?", (lead_id,))
                    cursor.execute(sql2, params2)
                    
                    sql3, params3 = adapt_placeholders("DELETE FROM outbound_leads WHERE id=?", (lead_id,))
