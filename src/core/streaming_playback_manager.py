...
class StreamingPlaybackManager:
            ...
            while True:
                ...
                if status == "error":
                    try:
                        ...
                        pass
                    break
                await self._update_idle_tracking(call_id, status)
        ...
        if info is not None:
            try:
                ...
                if not filler:
                    ...
                    # 🔇 Auto-mute channel when sending first real audio frame
                    # This prevents echo when agent starts speaking
                    channel_muted = bool(info.get('channel_muted', False))
                    first_real_frame_sent = bool(info.get('first_real_frame_sent', False))
                    
                    if not channel_muted and not first_real_frame_sent:
                        # This is the first real frame being sent - mute the channel
                        try:
                            session = await self.session_store.get_by_call_id(call_id)
                            if session and session.caller_channel_id:
                                success = await self.ari_client.mute_channel(
                                    session.caller_channel_id, 
                                    direction="in"
                                )
                                if success:
                                    info['channel_muted'] = True
                                    info['first_real_frame_sent'] = True
                                    info['channel_unmuted'] = False  # Reset unmute flag
                                    # Ensure flags persist
                                    self.active_streams[call_id] = info
                                    logger.info(
                                        "🔇 Channel muted (first audio frame sent) - flags: muted=True, unmuted=False",
                                        call_id=call_id,
                                        channel_id=session.caller_channel_id
                                    )
                                else:
                                    logger.warning(
                                        "Failed to mute channel on first frame",
                                        call_id=call_id,
                                        channel_id=session.caller_channel_id
                                    )
                        except Exception as e:
                            logger.error(
                                "Error muting channel on first frame",
                                call_id=call_id,
                                error=str(e),
                                exc_info=True
                            )
    ...
    async def _update_idle_tracking(self, call_id: str, status: str) -> None:
        """Update idle tracking and handle channel unmute when agent finishes speaking.
        
        Args:
            call_id: Unique call identifier
            status: Current pacer status ("wait", "sent", "finished", "error")
        """
        info = self.active_streams.get(call_id)
        if not info:
            return
        ...
        # Update idle tick counter based on status
        if status == "sent":
        ...
        # 🔓 Unmute channel when agent finishes speaking
        # Use idle time detection since Google Live may not emit AgentAudioDone reliably
        # Unmute after agent stops sending audio frames for a short period (500ms)
        channel_muted = bool(info.get('channel_muted', False))
        channel_unmuted = bool(info.get('channel_unmuted', False))
        last_real_emit_ts = info.get('last_real_emit_ts')
        
        # Calculate idle time since last real audio frame
        idle_ms = 0.0
        if last_real_emit_ts is not None:
            idle_ms = max(0.0, (time.time() - float(last_real_emit_ts)) * 1000.0)
        
        # Unmute threshold: 500ms of silence after audio playback
        unmute_delay_ms = 500
        should_unmute = (
            channel_muted 
            and not channel_unmuted 
            and last_real_emit_ts is not None  # Has sent at least one real frame
            and idle_ms >= unmute_delay_ms  # Idle for 500ms
        )
        
        # Debug logging to track unmute conditions
        # if channel_muted and not channel_unmuted:
        #     logger.info(
        #         "🔍 Unmute check in _update_idle_tracking",
        #         call_id=call_id,
        #         status=status,
        #         channel_muted=channel_muted,
        #         channel_unmuted=channel_unmuted,
        #         idle_ms=int(idle_ms),
        #         unmute_delay_ms=unmute_delay_ms,
        #         should_unmute=should_unmute,
        #         has_real_frame=(last_real_emit_ts is not None)
        #     )
        
        if should_unmute:
            # Agent audio is done (idle timeout) and channel is still muted - unmute it
            try:
                session = await self.session_store.get_by_call_id(call_id)
                if session and session.caller_channel_id:
                    success = await self.ari_client.unmute_channel(
                        session.caller_channel_id, 
                        direction="in"
                    )
                    if success:
                        info['channel_unmuted'] = True
                        # Ensure flags persist
                        self.active_streams[call_id] = info
                        logger.info(
                            "🔓 Channel unmuted (agent finished speaking - buffer empty)",
                            call_id=call_id,
                            channel_id=session.caller_channel_id,
                            idle_ms=int(idle_ms)
                        )
                        
                        # 🔓 Audio gating: Agent finished playing audio (buffer empty), open the gate
                        # Call engine callback if available
                        if hasattr(self, 'on_playback_complete_callback') and self.on_playback_complete_callback:
                            try:
                                await self.on_playback_complete_callback(call_id)
                            except Exception as e:
                                logger.error(
                                    "Error calling playback complete callback",
                                    call_id=call_id,
                                    error=str(e)
                                )
                        
                        # Reset mute flags for next segment
                        info['channel_muted'] = False
                        info['channel_unmuted'] = False
                    else:
                        logger.warning(
    ...
    async def mark_segment_boundary(self, call_id: str) -> None:
        ...
        try:
            ...
            # Note: Do NOT reset channel_muted/channel_unmuted here - they must persist
            # until end_segment_gating runs (which is called AFTER this function)
            info['sentinel_seen'] = False
            info['first_real_frame_sent'] = False
            ...
            logger.info(
                "⚡ CONTINUOUS STREAM - Segment boundary (flags reset)",
                call_id=call_id,
    ...
    async def end_segment_gating(self, call_id: str) -> None:
        ...
        NOTE: This is called when AgentAudioDone is received, but audio is still
        playing from buffer. DO NOT unmute here - unmute happens in _update_idle_tracking
        when buffer is actually empty (500ms idle).
        """
        try:
            info = self.active_streams.get(call_id)
            if not info:
                logger.debug("end_segment_gating - no active_streams info", call_id=call_id)
                return
            stream_id = str(info.get('stream_id') or '')
            if not stream_id:
                logger.debug("end_segment_gating - no stream_id", call_id=call_id)
                return
            
            logger.debug(
                "end_segment_gating - clearing TTS gating token only",
                call_id=call_id,
                stream_id=stream_id
            )
            
            # Only clear TTS gating token - unmute will happen in _update_idle_tracking
            # when buffer is actually empty
    ...
    async def _cleanup_stream(self, call_id: str, stream_id: str) -> None:
        ...
        try:
            ...
            # 🔓 CRITICAL: Unmute channel before cleanup if still muted
            # This ensures user can speak even if unmute didn't trigger during streaming
            try:
                if call_id in self.active_streams:
                    info = self.active_streams[call_id]
                    channel_muted = bool(info.get('channel_muted', False))
                    ...
                    if channel_muted and not channel_unmuted:
                        logger.info(
                            "🔍 Stream cleanup detected muted channel - forcing unmute",
                            call_id=call_id,
                            stream_id=stream_id
                        )
                        try:
                            ...
                            if session and session.caller_channel_id:
                                ...
                                if success:
                                    info['channel_unmuted'] = True
                                    logger.info(
                                        "🔓 Channel unmuted (stream cleanup)",
                                        call_id=call_id,
                                        channel_id=session.caller_channel_id
                                    )
                                else:
                                    logger.warning(
                                        "Failed to unmute channel during cleanup",
                                        call_id=call_id,
                                        channel_id=session.caller_channel_id
                                    )
                        except Exception as e:
                            logger.error(
                                "Error unmuting channel during cleanup",
                                call_id=call_id,
                                error=str(e),
                                exc_info=True
                            )
            except Exception:
                logger.debug("Cleanup unmute check failed", call_id=call_id, exc_info=True)
