...
class OpenAIRealtimeProvider(AIProviderInterface):
    ...
    async def _emit_audio_done(self):
        if not self.on_event or not self._call_id:
            logger.debug("_emit_audio_done: early return (no on_event or call_id)", call_id=self._call_id, has_on_event=bool(self.on_event))
            return
        try:
            logger.info("_emit_audio_done called", call_id=self._call_id, in_audio_burst=self._in_audio_burst)
            if self._in_audio_burst:
                logger.info("🎯 Emitting AgentAudioDone event", call_id=self._call_id)
                await self.on_event(
                    {
                        "type": "AgentAudioDone",
                        "streaming_done": True,
                ...
                logger.warning("🎯 NOT emitting AgentAudioDone - not in audio burst", call_id=self._call_id)
        except Exception:
