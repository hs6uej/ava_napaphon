...
class GoogleLiveProvider(AIProviderInterface):
    ...
    def __init__(
        ...
        self._call_metadata: Optional[Dict[str, Any]] = None
    ...
    async def _send_setup(self, context: Optional[Dict[str, Any]]) -> None:
        ...
        # system_prompt = None
        # if context:
        #     # Try "instructions" first (ElevenLabs/Google Live alias)
        #     system_prompt = context.get("instructions")
        #     # Fallback to "prompt" (standard context field)
        #     if not system_prompt:
        #         system_prompt = context.get("prompt")
    
        # # Final fallback to provider config
        # if not system_prompt:
        #    system_prompt = self.config.instructions
    ...
    async def _handle_server_message(self, data: Dict[str, Any]) -> None:
        ...
        # logger.info(
        #         "Received _handle_server_message (not implemented)", self._safe_jsonable(data)) 
    ...
    async def _handle_server_content(self, data: Dict[str, Any]) -> None:
        ...
        if content.get("interrupted") is True:
            ...
            try:
                if self.on_event:
                    logger.warning("!!!! skiped on_event ProviderBargeIn on interrupted due to in_audio_burst", call_id=self._call_id)
                    await self.on_event(
                        {
        ...
        if turn_complete:
            logger.info(
                "🎯 Google Live turnComplete received",
                call_id=self._call_id,
                in_audio_burst=self._in_audio_burst
            )
        ...
        if turn_complete:
            ...
            if assistant_final_text:
                ...
                # Note: Track conversation message after usage metadata is processed below
        ...
        for part in model_parts:
            ...
            # elif "usageMetadata" in part:
            #     usage = part["usageMetadata"]
            #     logger.info(
            #         "Google Live usage metadata from modelTurn",
            #         call_id=self._call_id,
            #         usage=self._safe_jsonable(usage),
            #    )
        ...
        if turn_complete:
            ...
            if "usageMetadata" in data:
                ...
                logger.info(
                    "Google Live usage metadata",
                    call_id=self._call_id,
                    usage=self._safe_jsonable(usage),
                )
                # Persist usage metadata into the session so PostCall tools can access it.
                # promptTokenCount is cumulative (full context window), so we always replace
                # it with the latest snapshot.  candidatesTokenCount is per-turn output, so
                # we accumulate a running total across turns.
                # session_store = getattr(self, "_session_store", None)
                # if session_store and self._call_id:
                #     try:
                #         session = await session_store.get_by_call_id(self._call_id)
                #         if session:
                #             prev_total_output = session.usage_metadata_summary.get("total_candidates_token_count", 0)
                #             # Keep raw latest snapshot
                #             session.usage_metadata = dict(usage)
                #             # Build accumulated summary
                #             session.usage_metadata_summary = dict(usage)
                #             session.usage_metadata_summary["total_candidates_token_count"] = (
                #                 prev_total_output + int(usage.get("candidatesTokenCount", 0))
                #             )
                #             await session_store.upsert_call(session)
                #     except Exception:
                #         logger.debug("Failed to save usage metadata to session", call_id=self._call_id, exc_info=True)
            
            # Track assistant message with usage metadata
            if assistant_final_text:
                await self._track_conversation_message("assistant", assistant_final_text, usage=usage_metadata)

            # 'usageMetadata': {'promptTokenCount': 7868, 'candidatesTokenCount': 283, 'totalTokenCount': 8151, 'promptTokensDetails': [{'modality': 'AUDIO', 'tokenCount': '422'}, {'modality': 'TEXT', 'tokenCount': '7446'}
    ...
    async def _handle_audio_output(self, audio_b64: str, *, mime_type: str = "") -> None:
        ...
        try:
            ...
            if not self._in_audio_burst:
                ...
                logger.info("🎯 Google Live audio burst started", call_id=self._call_id)
                ## LAM: Emit AgentSpeaking event at start of audio burst
                if self._gating_manager:
                   self._gating_manager.set_agent_speaking(self._call_id, True, "google_live")
                ## LAM: Emit AgentSpeaking event at start of audio burst
    ...
    async def _handle_turn_complete(self) -> None:
        ...
        logger.info(
            "🎯 Google Live _handle_turn_complete called",
            call_id=self._call_id,
            in_audio_burst=self._in_audio_burst,
            had_audio=had_audio,
        ...
        if self._in_audio_burst:
            self._in_audio_burst = False
            ## LAM: AgentAudioDone will trigger end_segment_gating in engine, which will call set_agent_speaking(False)
            ## Do NOT call set_agent_speaking(False) here - audio still in buffer!
            if self.on_event:
                logger.info("🎯 Google Live emitting AgentAudioDone", call_id=self._call_id)
                await self.on_event(
                    {
                        "type": "AgentAudioDone",
                        "call_id": self._call_id,
                ...
                logger.warning("🎯 Google Live NOT emitting AgentAudioDone - no on_event callback", call_id=self._call_id)
    ...
    async def _track_conversation_message(self, role: str, text: str, usage: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        Args:
            ...
            usage: Optional usage metadata (for assistant messages from Google Live)
        """
        ...
        try:
            ...
            if session:
                ...
                # Add usage metadata if available (for assistant messages)
