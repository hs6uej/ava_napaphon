...
class ExternalMediaEngine:
    ...
    async def _on_provider_event(self, event: Dict[str, Any]):
        ...
        try:
            ...
            # Agent TTS audio started: mute caller (remove from bridge) then forward agent audio
            if event_type == 'AgentAudio':
                ...
                if audio_data and call_id:
                    ...
                    if call_info:
                        ...
                        # Remove caller from bridge once at start of agent audio burst
                        if bridge_id and not call_info.get('muted_for_tts', False):
                            try:
                                await self.ari_client.remove_channel_from_bridge(bridge_id, call_id)
                                call_info['muted_for_tts'] = True
                                logger.info("Muted caller for TTS playback", call_id=call_id, bridge_id=bridge_id)
                            except Exception:
                                logger.debug("Failed to mute caller before TTS", call_id=call_id, exc_info=True)

                    # Forward agent audio to RTP as before
                    try:
                        ...
                        logger.error("Failed to forward AgentAudio to RTP", call_id=call_id, exc_info=True)

            # Agent TTS burst finished: re-add caller to bridge (unmute)
            elif event_type == 'AgentAudioDone':
                call_info = self.active_calls.get(call_id)
                if call_info and call_info.get('muted_for_tts', False):
                    bridge_id = call_info.get('bridge_id')
                    if bridge_id:
                        try:
                            await self.ari_client.add_channel_to_bridge(bridge_id, call_id)
                            call_info['muted_for_tts'] = False
                            logger.info("Unmuted caller after TTS", call_id=call_id, bridge_id=bridge_id)
                        except Exception:
                            logger.debug("Failed to unmute caller after TTS", call_id=call_id, exc_info=True)
    ...
    async def _start_ai_pipeline(self, call_id: str):
        ...
        try:
            ...
            self.active_calls[call_id] = {
                ...
                # Track whether we've muted the caller while the agent speaks
                "muted_for_tts": False,
            }
    ...
    async def _on_playback_finished(self, event: Dict[str, Any]):
        ...
        logger.info("ExternalMedia: Playback finished")
        try:
            ...
            if playback_id in self.ari_client.active_playbacks:
                ...
                logger.info("ExternalMedia: Playback finished and cleaned up", playback_id=playback_id)
                
        except Exception as e:
            logger.error(f"ExternalMedia: Error handling PlaybackFinished: {e}")
