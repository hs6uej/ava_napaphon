...
class ARIClient:
    ...
    async def mute_channel(self, channel_id: str, direction: str = "in") -> bool:
        """
        Mute audio on a channel.
        
        Args:
            channel_id: The channel ID to mute
            direction: Direction to mute - "in", "out", or "both" (default: "in")
            
        Returns:
            bool: True if mute was successful, False otherwise
        """
        if direction not in ["in", "out", "both"]:
            logger.error("Invalid mute direction", channel_id=channel_id, direction=direction)
            return False
            
        logger.info("Muting channel", channel_id=channel_id, direction=direction)
        ...
        # if response and response.get("status") and response.get("status") >= 400:
        #     logger.error("Failed to mute channel", channel_id=channel_id, direction=direction, response=response)
        #     return False
        
        logger.info("Channel muted successfully", channel_id=channel_id, direction=direction)
        return True

    async def unmute_channel(self, channel_id: str, direction: str = "in") -> bool:
        """
        Unmute audio on a channel.
        
        Args:
            channel_id: The channel ID to unmute
            direction: Direction to unmute - "in", "out", or "both" (default: "in")
               in: Mute audio coming into the PBX (microphone)
               out: Mute audio going out of the PBX (speaker)
               both: Mute both directions
            
        Returns:
            bool: True if unmute was successful, False otherwise
        """
        if direction not in ["in", "out", "both"]:
            logger.error("Invalid unmute direction", channel_id=channel_id, direction=direction)
            return False
            
        logger.info("Unmuting channel", channel_id=channel_id, direction=direction)
        response = await self.send_command(
            "DELETE", 
            f"channels/{channel_id}/mute",
            params={"direction": direction}
        ...
        # if response and response.get("status") and response.get("status") >= 400:
        #     logger.error("Failed to unmute channel", channel_id=channel_id, direction=direction, response=response)
        #     return False
        
        logger.info("Channel unmuted successfully", channel_id=channel_id, direction=direction)
        return True
