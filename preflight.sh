...
check_directories() {
    ...
    # Check data directory (for call history database)
    # Detect if using PostgreSQL or SQLite
    local DB_URL="${DATABASE_URL:-}"
    if [ -z "$DB_URL" ] && [ -f "$SCRIPT_DIR/.env" ]; then
        DB_URL=$(grep -E '^[^#]*DATABASE_URL=' "$SCRIPT_DIR/.env" 2>/dev/null | tail -n1 | sed -E 's/^[^#]*DATABASE_URL=//' | tr -d '"' | xargs)
    fi
    
    local using_postgres=false
    if [[ "$DB_URL" =~ ^postgresql:// ]]; then
        using_postgres=true
        log_ok "Database: PostgreSQL configured (DATABASE_URL set)"
    else
        log_info "Database: SQLite mode (data/call_history.db)"
    fi
    ...
    if [ -d "$DATA_DIR" ]; then
        ...
        # Only test SQLite if not using PostgreSQL
...
os.remove(path)
PY
                    log_ok "Call history DB: SQLite test passed"
                else
                    log_warn "Call history DB: SQLite test failed"
                    ...
                    log_info "  Consider migrating to PostgreSQL: ./scripts/setup_postgres.sh"
                fi
            fi
        else
            log_info "  Data directory maintained for SQLite fallback/migration compatibility"
        fi
    else
        if [ "$APPLY_FIXES" = true ]; then
            mkdir -p "$DATA_DIR"
            ...
            if [ "$using_postgres" = false ]; then
                ...
                log_info "  Recommended to create for SQLite fallback compatibility"
            fi
...
check_data_permissions() {
    ...
    # Only check SQLite files if not using PostgreSQL
    local DB_URL="${DATABASE_URL:-}"
    if [ -z "$DB_URL" ] && [ -f "$SCRIPT_DIR/.env" ]; then
        DB_URL=$(grep -E '^[^#]*DATABASE_URL=' "$SCRIPT_DIR/.env" 2>/dev/null | tail -n1 | sed -E 's/^[^#]*DATABASE_URL=//' | tr -d '"' | xargs)
    fi
    ...
    if [ "$using_postgres" = false ]; then
        # Check SQLite database files ownership
        ...
        # Using PostgreSQL, SQLite files not relevant
        if [ -f "$DATA_DIR/call_history.db" ]; then
            log_info "SQLite files present but using PostgreSQL (files preserved for backward compatibility)"
            log_info "  Run scripts/migrate_sqlite_to_postgres.py if you need to migrate old data"
        fi
    fi
}
...
check_asterisk_uid_gid() {
    ...
    if [ "$APPLY_FIXES" = true ]; then
        ...
        # This is required for persistent call history (SQLite) or for backward compatibility (PostgreSQL).
        ...
        # If the DB already exists (or Admin UI created WAL/SHM), ensure it is group-writable (SQLite only).
        # Otherwise ai-engine (runs as appuser) may fail with:
        #   sqlite3.OperationalError: attempt to write a readonly database
        # Skip this if using PostgreSQL
        local DB_URL="${DATABASE_URL:-}"
        if [ -z "$DB_URL" ] && [ -f "$SCRIPT_DIR/.env" ]; then
            DB_URL=$(grep -E '^[^#]*DATABASE_URL=' "$SCRIPT_DIR/.env" 2>/dev/null | tail -n1 | sed -E 's/^[^#]*DATABASE_URL=//' | tr -d '"' | xargs)
        fi
        ...
        if [[ ! "$DB_URL" =~ ^postgresql:// ]]; then
            # Using SQLite, fix file permissions
            ...
            log_info "Using PostgreSQL - SQLite file checks skipped"
        fi
        done
